...
num_steps = 400
sigmas = jnp.geomspace(1.0, 1e-3, num_steps)
lr     = 0.5 * sigmas**1.5          # 1.5 made all ones
noise_scale = 0.8                 # 0.7 was great

for i, (σ, η) in enumerate(zip(sigmas, lr)):
    rng, step_rng = jax.random.split(rng)

    t = jnp.full((batch.shape[0],), σ)
    s = model.apply(params, batch, t)

    batch += η * s
    batch += noise_scale * jnp.sqrt(2 * η) * jax.random.normal(step_rng, batch.shape)

    if i % 20 == 0:
        fig, ax = plt.subplots(4, 4, figsize=(8, 8))
        for k in range(16):
            r, c = divmod(k, 4)
            ax[r, c].imshow((batch[k, ..., 0] + 1) / 2, cmap='gray')
            ax[r, c].axis('off')
        plt.tight_layout()
        plt.show()
...
n = 6
m = 6
batch = jax.random.uniform(rng, (n * m, 28, 28, 1), minval=-1.0, maxval=1.0)  # better range for model

num_steps = 400
step_size = 0.0001                         # was 0.001 — slightly stronger update
noise_scale = 0.8                         # moderate noise to improve diversity

ts = jnp.geomspace(1.0, 1e-3, num_steps)

for ti, t in tqdm(enumerate(ts)):
    step_rng, rng = jax.random.split(rng)

    s = model.apply(params, batch, jnp.full((batch.shape[0],), 1e-3))  # fixed t=1e-3
    batch += step_size * s / 2
    batch += noise_scale  * jax.random.normal(step_rng, batch.shape)
    ...
    if ti % 20 == 0:
        ...
        for i in range(n * m):
            row, col = i // m, i % m
            axes[row, col].imshow((batch[i, ..., 0] + 1) / 2, cmap='gray')
            axes[row, col].axis('off')
        plt.tight_layout()
        plt.show()
